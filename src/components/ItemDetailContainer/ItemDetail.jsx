   ...
   /*  const { id } = useParams() */
    ...
    const onAdd = (count) => {
        ...
        Swal.fire({
            ...
            text: `Agregaste ${count} ${item.lastname} al carrito: ${item.name} ${item.lastname}`,
            imageUrl: `${item.image}`,
            imageWidth: 200,
            imageHeight: 200,
    ...
    return (
        <div className='DetailContainer' key={item.id}>
            <div className='cardDetail'>
                <h1><strong> {item.lastname}</strong></h1>
                <img className='imgLego' src={item.image} alt={item.lastname} />
